...
RSpec.describe 'UpdatingUsers', type: :system do
  ...
  context 'when user signed in is admin' do
    let(:create_user) { create(:user, user_type: 'Broker', approved: false, confirmed_at: nil) }
    ...
    it 'approves a broker user' do
      expect(User.find_by(email: email).approved).to eq(true)
    end
  end
